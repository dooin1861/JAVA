...
String deleteNum = request.getParameter("deleteA"); // delete시 넘어오는 매개변수
//out.println(find);
String num = request.getParameter("num");  //  -> update, insert를 위해 선언
...
if (num != null) {
	sql = "select count(*) from score where num = ?";  // 0이면 insert, 1이면 update
	...
	if (cnt == 1) { // 정보 수정
		...
		sql = "insert into score(num, name, kor, eng, math) values (?,?,?,?,?)";  // insert
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, num);
...
while(rs.next()) {
	...
	DecimalFormat df = new DecimalFormat("#.##"); // 소수점 2자리만
	double avg = tot / 3.0;
%>
<tr>
