...
String num = request.getParameter("num");            // null이면 insert, update를 하지 말아야함
...
if (num != null) {
	sqlTmp = "select count(*) from score where num = ?";  // 이미 있는 학번 조회 (있으면 1, 없으면 0을 가져옴)
	pstmt = conn.prepareStatement(sqlTmp);
	...
	if (cnt == 1) { // 정보 수정
		...
		sqlTmp = "insert into score(num, name, kor, eng, math) values (?,?,?,?,?)";  // insert
		pstmt = conn.prepareStatement(sqlTmp);
		pstmt.setString(1, num);
